...
const getArrayValueLength = (object, json) => {
  if(typeof json === 'string'){
    const arrayLocation = json.split('.$').shift();
    return get(object, arrayLocation, []).length;
  }

  if(typeof json === 'object'){
    return Object.keys(json).reduce((maxLength, key) => {
      return Math.max(maxLength, getArrayValueLength(object, json[key]));
    }, -1);
  }
...
const fillKeys = (json, object, undefinedValue, opts) => {
  ...
  Object.keys(json).forEach(key => {
    ...
    if(Array.isArray(json[key])) {
      ...
      const arrayValueLength = getArrayValueLength(object, json[key][0]);
      const arrayValue = [];
